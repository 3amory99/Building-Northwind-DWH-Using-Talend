...
package northwind_dwh.dim_date_0_1;
...
public class Dim_Date implements TalendJob {
	...
	public class ContextProperties extends PropertiesWithType {
		...
		public void synchronizeContext() {
			...
			if (start_date != null) {
				...
				String value_start_date = "1996-07-04 00:00:00.000";
				String[] parts_start_date = value_start_date.split(";");
				if (parts_start_date.length > 1) {
					pattern_start_date = parts_start_date[0];
					this.setProperty("start_date",
							pattern_start_date + ";" + FormatterUtils.format_Date(start_date, pattern_start_date));
				} else {
	...
	public static class row2Struct implements routines.system.IPersistableRow<row2Struct> {
		final static byte[] commonByteArrayLock_NORTHWIND_DWH_Dim_Date = new byte[0];
		static byte[] commonByteArray_NORTHWIND_DWH_Dim_Date = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_NORTHWIND_DWH_Dim_Date.length) {
					if (length < 1024 && commonByteArray_NORTHWIND_DWH_Dim_Date.length == 0) {
						commonByteArray_NORTHWIND_DWH_Dim_Date = new byte[1024];
					} else {
						commonByteArray_NORTHWIND_DWH_Dim_Date = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_NORTHWIND_DWH_Dim_Date, 0, length);
				strReturn = new String(commonByteArray_NORTHWIND_DWH_Dim_Date, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_NORTHWIND_DWH_Dim_Date, 0, length);
				strReturn = new String(commonByteArray_NORTHWIND_DWH_Dim_Date, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_NORTHWIND_DWH_Dim_Date) {
	...
	public void tRowGenerator_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				util_tLogRow_1.addRow(new String[] { "Date_SK", "Date_ID", "Day", "Month", "Year", });
...
///////////////////////    			
				...
				final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:+1ahjq2sgcXlZPhbtNNNx4BOT/oGaihiHi1mzg==");
				...
				if (!whetherExist_tDBOutput_1) {
					try (java.sql.Statement stmtCreate_tDBOutput_1 = conn_tDBOutput_1.createStatement()) {
						stmtCreate_tDBOutput_1.execute("CREATE TABLE [" + tableName_tDBOutput_1
								+ "]([Date_SK] INT  not null ,[Date_ID] DATETIME  not null ,[Day] VARCHAR(10)   not null ,[Month] VARCHAR(10)   not null ,[Year] VARCHAR(10)   not null ,primary key([Date_SK]))");
					}
				}
				java.sql.PreparedStatement pstmt_tDBOutput_1 = conn_tDBOutput_1
						.prepareStatement("SELECT COUNT(1) FROM [" + tableName_tDBOutput_1 + "] WHERE [Date_SK] = ?");
				resourceMap.put("pstmt_tDBOutput_1", pstmt_tDBOutput_1);
				String insert_tDBOutput_1 = "INSERT INTO [" + tableName_tDBOutput_1
						+ "] ([Date_SK],[Date_ID],[Day],[Month],[Year]) VALUES (?,?,?,?,?)";
...
// ###############################
				...
				int nb_max_row_tRowGenerator_1 = 8000;

				class tRowGenerator_1Randomizer {
					public java.util.Date getRandomDate() {

						return TalendDate.addDate(context.start_date, Numeric.sequence("s1", 1, 1) - 1, "DD");
				...
				for (int itRowGenerator_1 = 0; itRowGenerator_1 < nb_max_row_tRowGenerator_1; itRowGenerator_1++) {
					...
					{ // start of Var scope
						...
						Var.Date_SK = Numeric.sequence("s2", 1, 1);// ###############################
						// ###############################
						// # Output tables
...
// # Output table : 'out1'
						out1_tmp.Date_SK = Var.Date_SK;
						out1_tmp.Date_ID = row1.Date;
						out1_tmp.Day = TalendDate.formatDate("dd", row1.Date);
						out1_tmp.Month = TalendDate.formatDate("MM", row1.Date);
						out1_tmp.Year = TalendDate.formatDate("yyyy", row1.Date);
						out1 = out1_tmp;
// ###############################
...
// Start of branch "out1"
					if (out1 != null) {
						...
						row2.Date_SK = out1.Date_SK;
						row2.Date_ID = out1.Date_ID;
						row2.Day = out1.Day;
						row2.Month = out1.Month;
						row2.Year = out1.Year;
						row3 = new row3Struct();

						row3.Date_SK = out1.Date_SK;
						row3.Date_ID = out1.Date_ID;
						row3.Day = out1.Day;
						row3.Month = out1.Month;
						row3.Year = out1.Year;
...
///////////////////////		
						...
						row_tLogRow_1[0] = String.valueOf(row2.Date_SK);

						if (row2.Date_ID != null) { //
							row_tLogRow_1[1] = FormatterUtils.format_Date(row2.Date_ID, "dd-MM-yyyy");

						} //

						if (row2.Day != null) { //
							row_tLogRow_1[2] = String.valueOf(row2.Day);

						} //

						if (row2.Month != null) { //
							row_tLogRow_1[3] = String.valueOf(row2.Month);

						} //

						if (row2.Year != null) { //
							row_tLogRow_1[4] = String.valueOf(row2.Year);
...
///////////////////////    			
						...
						pstmt_tDBOutput_1.setInt(1, row3.Date_SK);
						...
						if (checkCount_tDBOutput_1 < 1) {
							pstmtInsert_tDBOutput_1.setInt(1, row3.Date_SK);

							if (row3.Date_ID != null) {
								pstmtInsert_tDBOutput_1.setTimestamp(2, new java.sql.Timestamp(row3.Date_ID.getTime()));
							} else {
							...
							if (row3.Day == null) {
								...
								pstmtInsert_tDBOutput_1.setString(3, row3.Day);
							}

							if (row3.Month == null) {
								pstmtInsert_tDBOutput_1.setNull(4, java.sql.Types.VARCHAR);
							} else {
								pstmtInsert_tDBOutput_1.setString(4, row3.Month);
							}

							if (row3.Year == null) {
								pstmtInsert_tDBOutput_1.setNull(5, java.sql.Types.VARCHAR);
							} else {
								pstmtInsert_tDBOutput_1.setString(5, row3.Year);
							}
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = Dim_Date.class.getClassLoader()
					.getResourceAsStream("northwind_dwh/dim_date_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
					...
					if (context.getStringValue("start_date") == null) {
						...
						try {
							...
							if (context_start_date_pos > -1) {
								context_start_date_pattern = context_start_date_value.substring(0,
										context_start_date_pos);
								context_start_date_value = context_start_date_value
										.substring(context_start_date_pos + 1);
							}

							context.start_date = (java.util.Date) (new java.text.SimpleDateFormat(
									context_start_date_pattern).parse(context_start_date_value));

						} catch (ParseException e) {
							try {
								long context_start_date_longValue = Long.valueOf(context_start_date_value);
								context.start_date = new java.util.Date(context_start_date_longValue);
								...
								System.err.println(String.format("Null value will be used for context parameter %s: %s",
										"start_date", "Can't parse date string: " + e.getMessage() + " and long: "
...
/************************************************************************************************
 * 88939 characters generated by Talend Open Studio for Data Integration on the
 * November 11, 2023 at 2:15:51 AM EET
 ************************************************************************************************/